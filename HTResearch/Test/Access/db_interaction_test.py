...
class DatabaseInteractionTest(unittest.TestCase):
    ...
    def test_organization_dao(self):
        ...
        assert_orgs = org_dao.text_search(num_elements=10, text='bEe YeE university ers', fields=['name', 'address', 'organization_url', ])
        self.assertEqual(assert_orgs[0].name, org_dto.name)

        assert_orgs = org_dao.text_search(num_elements=10, text='yee adfgh905w', fields=['name', 'address', 'organization_url', ])
